...
function createSuggestion() {
  ...
  li.classList.add('app-autocomplete-search__suggestion')
...
function manageChoiceHighlight($suggestion, index, suggestionIndex = null) {
  const className = 'app-autocomplete-search__suggestion--highlight'
...
function buildNoResults() {
  ...
  li.classList.add(
    'app-autocomplete-search__suggestion',
    'app-autocomplete-search__suggestion--no-results'
  )
  li.setAttribute('role', 'option')
...
function autocompleteSearch($module) {
  if (!$module) {
    return
  }

  const $input = $module.querySelector(
    `[data-js="app-autocomplete-search-input"]`
  )
  ...
  const $clearButton = $module.querySelector(
    `[data-js="app-autocomplete-search-clear-button"]`
  )
  const $suggestionsContainer = $module.querySelector(
    `[data-js="app-autocomplete-search-suggestions"]`
  )

  const isSuggestionsHidden = !$suggestionsContainer.classList.contains(
    'app-autocomplete-search__suggestions--show'
  )
  ...
  const showCloseButton = () => {
    $clearButton.classList.add('app-autocomplete-search__clear-button--show')
    $clearButton.setAttribute('aria-hidden', 'false')
  }

  const hideCloseButton = () => {
    $clearButton.classList.remove('app-autocomplete-search__clear-button--show')
