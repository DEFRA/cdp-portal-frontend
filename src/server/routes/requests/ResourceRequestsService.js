...
export async function getRecentNonPendingResourceRequests(teamIds = []) {
  ...
  const endpoint = `${config.get('portalBackendUrl')}/resources/requests?modifiedAfter=${OneDayOld}&status=merged&status=closed&status=failed&status=done${teamFilters}`
