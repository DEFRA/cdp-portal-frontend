...
function updateSecretController(serviceOrTestSuite) {
  return {
    options: {
      id: `post:${serviceOrTestSuite}s/{serviceId}/secrets/{environment}/update`,
    ...
    handler: async (request, h) => {
      ...
      const redirectUrl = request.routeLookup(
        `${serviceOrTestSuite}s/{serviceId}/secrets/{environment}/update`,
        {
      ...
      if (!validationResult.error) {
        const selfServiceOpsAddSecretEndpointUrl =
          ...
          `/${serviceOrTestSuite}s/add/${serviceId}/${environment}`
        ...
        try {
          ...
          return h.redirect(
            request.routeLookup(
              `${serviceOrTestSuite}s/{serviceId}/secrets/{environment}`,
              {
                params: { serviceId, environment }
              }
            )
