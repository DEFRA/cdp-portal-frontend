...
import { requestShutterUpdate } from '~/src/server/services/service/shuttering/helpers/fetchers.js'

const updateShutteringStatusController = {
  options: {
    id: 'post:services/{serviceId}/shuttering/confirm',
    auth: {
      mode: 'required',
      access: {
        scope: [
  ...
  handler: async (request, h) => {
    ...
    try {
      await requestShutterUpdate(request, sanitisedPayload)
    ...
    return h.redirect(
      request.routeLookup('services/{serviceId}/shuttering', {
        params: { serviceId }
      })
    )
  }
