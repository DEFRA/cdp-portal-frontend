...
import { addScopeToUser } from '../../../helpers/fetchers.js'
...
const addPermissionToUserController = {
  ...
  handler: async (request, h) => {
    ...
    const scopeToTeam = stepData.scopeKind === 'team'

    const validationResult = userPermissionValidation.validate(
      {
        userId,
        scopeId,
        ...(scopeToTeam && { teamId: stepData.teamId })
      },
      { abortEarly: false }
    )
    ...
    if (!validationResult.error) {
      try {
        await addScopeToUser({
          ...
          userId,
          scopeId,
          payload: { teamId: stepData.teamId }
        })
      } catch (error) {
      ...
      request.yar.flash(sessionNames.notifications, {
        text: 'Permission added to user successfully',
        type: 'success'
      })
