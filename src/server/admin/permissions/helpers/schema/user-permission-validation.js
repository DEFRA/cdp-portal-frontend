...
function userScopeValidation(scopeToTeam) {
  if (scopeToTeam) {
    return Joi.object({
      scopeKind: Joi.string().required().allow('team'),
  ...
  return Joi.object({
    scopeKind: Joi.string().required().allow('user').messages({
      'string.base': validation.chooseAnEntry,
      'any.required': validation.chooseAnEntry
    }),
    teamId: Joi.string().uuid(), // In this case teamId will be ignored when user scopeKind is processed
    redirectLocation: Joi.string().valid('summary', '')
  })
}
...
export { userPermissionValidation, userScopeValidation, userFindValidation }
